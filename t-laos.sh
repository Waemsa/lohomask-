...
rubygem_d &> /dev/null
termux-wake-lock;
if [ -e $PREFIX/share/figlet/Reno12M.flf ]; then
	echo -e "\e[1;34m[*] \033[32mReno12M.flf figlet font is present\033[0m";
	sleep 4
else
wget https://raw.githubusercontent.com/remo7777/REMO773/master/Remo773.flf &> /dev/null;
sleep 3
cp Reno12M.flf $PREFIX/share/figlet/Reno12M.flf;
...
tput sgr 0
if [[ ${PROC32} == [Y/y] ]]; then
	if [ -e $HOME/t-laos.txt ]; then
		rm $HOME/t-laos.txt;
	fi

	if [ -d $HOME/T-Laos ]; then
	cd $HOME/T-Laos
	fi
...
clear;
echo -e "\033[31m$(figlet -f Reno12M "T- LAOS")\e[0m"
echo -e "\e[1;32m
+----------------------------------*/
Reno12M : (\e[33m14.4.2021\e[32m)
